...
import { ME, SEED, day, type Status, type Type, type WorkItem } from "./data";
...
/**
 * #24's whole question is which corpus you are looking at, so the seed is bent three
 * ways: nothing ever created, everything settled, and a populated tree whose dates all
 * sit past ADR-0009's 30-day horizon.
 */
export function corpusFor(state: string): WorkItem[] {
  const seed = SEED.map((i) => ({ ...i }));
  if (state === "first-run") return [];
  if (state === "all-settled") return seed.map((i) => ({ ...i, status: "Completed" as Status }));
  // "Radar clear": a populated, healthy tree whose dated work all sits past the horizon.
  if (state === "clear") return seed.map((i) => ({ ...i, due: i.due ? day(38 + (i.id % 30)) : null }));
  return seed;
}

export function TreeProvider({ children: kids, state }: { children: React.ReactNode; state: string }) {
  const [items, setItems] = useState<WorkItem[]>(() => corpusFor(state));
