...
Creation has exactly one entry point per layout — the action button in compact, the sidebar button in split — and both open the same dialog: a summary, and a parent picker that decides the type. An empty parent means a Topic. **Amended by ADR-0018: the dialog leads with an explicit type control, and the parent list is filtered by the chosen type.** A Topic is now a type you pick rather than a parent you omit, and the dialog offers the whole core field set — Type, Summary and Parent required, the rest optional. The single-entry-point rule is untouched: one dialog still reaches every rung, and the Work Items tree's row menu opens that same dialog pre-filled rather than a second one. There is no separate "New Topic" affordance, because a second control for the same verb would have to explain why the ladder's top rung is created differently from every other rung.

**The action button is suppressed on Settings.** Settings holds no work items, so a standing invitation to create one there points at nothing on screen — and the button sits directly above the capsule tab that is already lit. The route declares it away with `export const handle = { fab: "none" }` and the shell reads `useMatches()`, the same mechanism Search uses for its layout (ADR-0022), so the shell never learns a route by name. The split layout's sidebar button is untouched: it is chrome beside the content rather than over it, and creation still has exactly one entry point per layout wherever it is offered.
...
Labels are absent from Due tab rows and Work Items tree rows. They are filter vocabulary rather than identity, and the chips crowded the summaries, which are the thing being scanned. Labels remain on the work item detail view, on Search rows, and in Settings — the three places where they are being read or chosen rather than passed over. **Amended by ADR-0018: labels are displayed only on the detail view, and carry no colour.** They are offered in the Search filter bar as criteria, but no row anywhere advertises which labels an item holds.

The compact row is now the constrained surface. A tree row carrying status, assignee and due date leaves little width for the summary at 390 points, and summaries truncate. What a row sheds at that width is the tree's decision, not the shell's. **Answered by ADR-0018: the compact row stacks rather than sheds** — summary on the first line, status, assignee and due date on a small second line.

Whether the Work Items tree also offers contextual creation — a plus on a Project that pre-fills the parent — is left open, but the single creation dialog must remain able to reach every rung on its own. **Answered by ADR-0018: the tree offers it from a row `⋯` menu rather than a visible plus**, and it opens the one dialog with type and parent already set.
