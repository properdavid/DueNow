...
export default function TreeA({ empty, onOpen, selected, compact, onMove, requestCreate }: TreeProps) {
  const t = useTree();
  ...
  const liveTopics = topics.filter(unfinished);
  const settledTopics = topics.filter(terminal);
  // ADR-0024: the test is whether any work item exists at all, never whether a row is
  // currently visible. Top-level terminal Topics hide behind the same per-parent reveal
  // every other rung uses, so "all settled" really is zero rows plus a reveal.
  const firstRun = t.items.length === 0;
  const rootRevealed = revealed.has(0);
  const nothingShown = liveTopics.length === 0 && (!rootRevealed || settledTopics.length === 0);
  ...
  return (
    <div className="pb-6" onClick={(e) => { if (!(e.target as HTMLElement).closest("[aria-label='Row menu']")) setMenu(null); }}>
      ...
      {nothingShown ? (
        <empty.TreeEmpty
          firstRun={firstRun}
          settled={settledTopics.length}
          onReveal={() => setRevealed((s) => new Set(s).add(0))}
          onCreate={() => requestCreate(null)}
          onCreateTopic={(summary) => t.create(null, summary, { type: "Topic" })}
          compact={compact}
        />
      ) : (
        <ul className="border-t border-line">
          {liveTopics.map((topic) => <Row key={topic.id} item={topic} depth={0} />)}
          {rootRevealed && settledTopics.map((topic) => <Row key={topic.id} item={topic} depth={0} />)}
          {!rootRevealed && settledTopics.length > 0 && (
            <li>
              <button
                onClick={() => setRevealed((s) => new Set(s).add(0))}
                className="w-full border-b border-line px-3 py-1 text-left text-[12px] text-faint hover:bg-surface"
              >
                {settledTopics.length} settled — show
              </button>
            </li>
          )}
        </ul>
