...
import { daysOut } from "../data";

export default function DueFixed({ empty, onOpen, selected, compact, requestCreate }: DueProps) {
  const t = useTree();
...
/** The nearest dated, unfinished work item sitting *past* ADR-0009's 30-day horizon. */
function nextBeyondHorizon(t: ReturnType<typeof useTree>) {
  const dated = t.items
    .filter((i) => unfinished(i) && i.due != null && daysOut(i.due) > 30)
    .sort((a, b) => (a.due! < b.due! ? -1 : 1));
  return dated.length ? { item: dated[0], days: daysOut(dated[0].due!) } : null;
}
