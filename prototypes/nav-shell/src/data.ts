...
export const ITEMS: WorkItem[] = [
  ...
  { id: 11, type: "Task", parentId: 10, summary: "Get patio cover quotes", status: "In Progress", assignee: "Dave", due: day(-2), labels: ["urgent"],
    ...
    comments: [{ author: "Dave", at: "3 days ago", body: "Two in. Third hasn't called back." }] },
...
export function children(id: number | null): WorkItem[] {
  return ITEMS.filter((i) => i.parentId === id);
}

export function descendants(id: number): WorkItem[] {
  const out: WorkItem[] = [];
...
export function ancestors(id: number): WorkItem[] {
  const out: WorkItem[] = [];
  let cur = byId.get(id);
...
export function lineage(id: number): string {
  return ancestors(id).map((a) => a.summary).join(" › ");
}
...
/** ADR-0009: unfinished, dated within 30 days, and not covered by a deeper unfinished item. */
export function dueRadar() {
  const dated = ITEMS.filter((i) => unfinished(i) && i.due && daysOut(i.due) <= 30);
  const visible = dated.filter((i) => {
    const covering = descendants(i.id).filter(
      (d) => unfinished(d) && d.due && dated.includes(d) && d.due <= i.due!,
    );
    return covering.length === 0;
  });
  const sorted = [...visible].sort((a, b) => (a.due! < b.due! ? -1 : a.due! > b.due! ? 1 : a.id - b.id));
  return {
    now: sorted.filter((i) => daysOut(i.due!) <= 0),
    soon: sorted.filter((i) => daysOut(i.due!) > 0 && daysOut(i.due!) <= 7),
    later: sorted.filter((i) => daysOut(i.due!) > 7),
  };
}
