...
// #24 asks about screens with nothing in them, so the black strip's second control is
// an app *state* switch rather than a data-volume one: signed out, an allowlist
// rejection, a first run, an all-settled tree, a clear radar, and a populated app to
...
export const VARIANTS = [
  { key: "A", name: "Signpost — every empty screen is a centred card with a way forward" },
  { key: "B", name: "Bare — one dim line in place: no card, no illustration, no button" },
  { key: "C", name: "Working — the empty space carries the next act or the next fact" },
] as const;
...
export function useNav() {
  ...
  const state = (params.get("state") ?? "first-run") as AppState;
